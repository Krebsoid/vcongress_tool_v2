...
@RequestScoped
public class EventRegistrationMail {
    ...
    public void onPersonEventRegistration(@Observes PersonEventRegistration personEventRegistration) {
        if(personEventRegistration.event.shortVersion()) {
            String registrationMail = "registration_short";
            String topic = localePropertyManager.get().getLocalizedProperty("app.event.registration.short.mail.topic");
            ...
            mailService.newMail().setSender("support@vcongress.de", "vCongress Support")
                    .setReceiver(new MailReceiver(personEventRegistration.person.user().name()).getValue())
                    .setTextContent(templateService.processTemplate(plainTemplate, personEventRegistration))
                    .setHtmlContent(templateService.processTemplate(htmlTemplate, personEventRegistration))
                    ...
                    .setTopic(personEventRegistration.event.name() + " - " + topic)
                    .sendMail();
        }
    }
    ...
    public void onTestSend(@Observes MailFeatureTest mailFeatureTest) {
        if(mailFeatureTest.eventFeature().eventFeatureType().equals(EventFeatureType.MAIL_REGISTRATION)) {
            PersonEventRegistration personEventRegistration = new PersonEventRegistration();
            personEventRegistration.event = mailFeatureTest.eventFeature().event();
            personEventRegistration.person = person;
            personEventRegistration.person.user().name(mailFeatureTest.getReceiver());
            this.onPersonEventRegistration(personEventRegistration);
        }
    }
